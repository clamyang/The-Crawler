...
class CrackGeetest():
    ...
    def get_screenshot(self):
        ...
        """
        if self.full > 0:
            self.browser.execute_script('document.getElementsByClassName("geetest_canvas_fullbg")[0].style="display: block; opacity: 1"')
            self.full = -1
        else:
    ...
    def crack(self):
        ...
        #获取带缺口的验证码图片
        image2 = self.get_geetest_image('capthca2.png')
        ...
        success = self.wait.until(EC.text_to_be_present_in_element((By.CLASS_NAME, 'geest_success_radar_tip_content'), '验证成功'))
        print(success)
